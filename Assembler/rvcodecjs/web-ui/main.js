...
const txtMult = document.getElementById('txtMult');
...
function renderConversion(inst, abi = false) {
  ...
  document.getElementById('hex-data').innerText = inst.hex;
...
txtHex.onclick = (e) => {
  var insts = txtMult.value.split('\n');
  var insHexs = [];
  insts.pop();
  insts.forEach((insAsm) => {
    const inst = new Instruction(insAsm,
      {
        ABI: abiParameter.checked,
        ISA: COPTS_ISA[isaParameter.value]
      });
    insHexs.push(inst.hex);
  })
  txtHex.value = insHexs.join('\n');
  navigator.clipboard.writeText(txtHex.value);
